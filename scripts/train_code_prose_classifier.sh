#!/bin/bash
# Train a FastText classifier for code prose educational quality.
#
# Data source: s3://ai2-llm/classifiers/code-quality/data/code_prose/2G_sample_annotated
# Annotation rubric: code_prose_edu (scores 1-5)
# Text field: .text (website snippets with code and prose)
#
# Pipeline:
...
#   ./scripts/train_code_prose_classifier.sh
#
# With custom local base directory:
#   LOCAL_BASE_DIR=/path/to/workdir ./scripts/train_code_prose_classifier.sh
#
...
# Rubric field (from code_prose_edu annotation)
RUBRIC_FIELD="code_prose_edu"

# Text expression: text is in the default .text field
TEXT_EXPRESSION=".text"

# Metadata field name for inference output
METADATA_FIELD="code_prose_${NORMALIZER}_bin5"

# Minimum valid samples to use valid set for calibration (otherwise use train)
MIN_VALID_FOR_CALIBRATION=5000

# S3 paths
S3_INPUT="${S3_INPUT:-s3://ai2-llm/classifiers/code-quality/data/code_prose/2G_sample_annotated}"
S3_BASE="${S3_BASE:-s3://ai2-llm/classifiers/code-quality}"
S3_OUTPUT="${S3_OUTPUT:-${S3_BASE}/trained_models/fasttext/code_prose_${NORMALIZER}_bin5}"
S3_SPLIT_DATA="${S3_SPLIT_DATA:-${S3_BASE}/data-train_test_split/code_prose}"
S3_PREPROCESSED="${S3_PREPROCESSED:-${S3_BASE}/preprocessed/code_prose/fasttext/${NORMALIZER}_bin5}"
S3_CALIBRATION="${S3_CALIBRATION:-${S3_BASE}/calibration/code_prose}"

# Local paths
LOCAL_BASE_DIR="${LOCAL_BASE_DIR:-${HOME}/ai2-llm/classifiers/code-quality}"
DATA_DIR="${LOCAL_BASE_DIR}/data/code_prose/2G_sample_annotated"
SPLIT_DATA_DIR="${LOCAL_BASE_DIR}/data-train_test_split/code_prose"
PREPROCESSED_DIR="${LOCAL_BASE_DIR}/preprocessed/code_prose/fasttext/${NORMALIZER}_bin5"
MODELS_DIR="${LOCAL_BASE_DIR}/trained_models/fasttext/code_prose_${NORMALIZER}_bin5"
CALIBRATION_DIR="${LOCAL_BASE_DIR}/calibration/code_prose"
...
log "Starting code prose classifier training pipeline"
...
# ==============================================================================
# Step 2: Split data into train/valid/test
# ==============================================================================
...
if [[ -f "${PREPROCESSED_DIR}/train.txt" ]]; then
    ...
    # Label expression: scores 1-5 map directly to bin1-bin5
...
echo "Calibration Results - Code Prose Classifier" > "${results_file}"
echo "============================================" >> "${results_file}"
echo "" >> "${results_file}"
